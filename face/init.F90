...
subroutine init_gamma_pulse
  ...
  do k=1,nspc
    ...
    if(Gamma_in_pulse_type(k).eq."S") then
        Gamma_in_pulse(k)=Gamma_in_pulse_S

    elseif (Gamma_in_pulse_type(k).eq."N") then
        Gamma_in_pulse(k)=Gamma_in_pulse_N

    elseif (Gamma_in_pulse_type(k).eq."B") then
        Gamma_in_pulse(k)=Gamma_in_pulse_B

    elseif (Gamma_in_pulse_type(k).eq."R") then
        Gamma_in_pulse(k)=Gamma_in_pulse_R

    else
        call face_error("Unknown Gamma_in_pulse::",Gamma_in_pulse_type(k),"at k=",k)
    endif
enddo
