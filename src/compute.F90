...
!        elseif (solve_heat_eq) then
    ...
    subroutine compute_surface_flx(k)
        ...
        elseif (left_surface_model_int(k).eq.surf_model_N) then
            ...
            call face_error("unknown left surface model:",left_surface_model_int(k))
        endif
        ...
        elseif (right_surface_model_int(k).eq.surf_model_N) then
            ...
            call face_error("unknown right surface model:",right_surface_model_int(k))
        endif
...
if(compute_spc) then
        ! pulsed plasma flow
         do k=1,nspc
!             if (Gamma_in_pulse_int(k).eq.Gamma_in_pulse_R) then !R
...
!                 endif
!             elseif (Gamma_in_pulse_int(k).eq.Gamma_in_pulse_N) then ! no pulse
!                 inflx(k)=Gamma_in_base(k)
! 
!             elseif (Gamma_in_pulse_int(k).eq.Gamma_in_pulse_S) then
! 
...
!         qflx_in=0.d0
! 
!         if (Q_in_pulse_int.eq.Q_in_pulse_R) then
!                 ! check that the pulse period is not zero
...
!                 endif
! 
!             elseif (Q_in_pulse_int.eq.Q_in_pulse_N) then ! no pulse
! 
!                 qflx_in=Q_in_base
!                 if (verbose_debug) then
!                 write(iout,*) 'no pulse: qflx_in=',qflx_in
!                 endif
!             elseif (Q_in_pulse_int.eq.Q_in_pulse_S) then
! 
...
! 
!             if (T_pulse_int.eq.T_pulse_N) then
...
!                 endif
! 
!             elseif (T_pulse_int.eq.T_pulse_S) then
! 
...
!            endif
    ...
    real function compute_dt() result(dt)
        use header
        implicit none
        integer  ::i, j, k
        real,parameter:: rtfm=5.e-1, rtfp=5.e-1
        real tmp
        tmp=0d0
        dt = 1e99
...
!        endif
           ...
           if(compute_spc) then
            ...
            do k=1,nspc
                do j=0,ngrd
                    if (abs(rate_d(ndt,j,k)) .gt. eps_machine) then
                        if (rate_d(ndt,j,k) .lt. 0.0) then
                            tmp=-rtfm*dens(ndt,j,k)/rate_d(ndt,j,k)*reduction_factor_dt_spc
                        else
                            tmp= rtfp*dens(ndt,j,k)/rate_d(ndt,j,k)*reduction_factor_dt_spc
                        endif
                    else
                        tmp=dt
                    endif
               if ((dt .gt. tmp) .and. (tmp .ne. 0.0)) then
                        dt=tmp
                    endif
                enddo
               ! left surface
                if ((left_surface_model_int(k).eq.surf_model_S)) then
                    if (Gsrf_l (ndt  ,k) .ne. 0.d0) then
                        if (Gsrf_l (ndt  ,k) .lt. 0.d0) then
                            tmp=-rtfm*dsrfl(ndt,k)/Gsrf_l (ndt  ,k)*reduction_factor_dt_spc
                        else
                            tmp= rtfp*dsrfl(ndt,k)/Gsrf_l (ndt  ,k)*reduction_factor_dt_spc
                        endif
                    else
                        tmp=dt
                    endif
                ...
                if ((right_surface_model_int(k).eq.surf_model_S) ) then
                    if (Gsrf_r (ndt  ,k) .ne. 0.d0) then
                        if (Gsrf_r (ndt  ,k) .lt. 0.d0) then
                            tmp=-rtfm*dsrfr(ndt,k)/Gsrf_r (ndt  ,k)*reduction_factor_dt_spc
                        else
                            tmp= rtfp*dsrfr(ndt,k)/Gsrf_r (ndt  ,k)*reduction_factor_dt_spc
                        endif
                    else
                        tmp=dt
                    endif
                ...
                do j=0,ngrd-1
                    if (rate_t(ndt,j) .ne. 0.d0) then
                        if (rate_t(ndt,j) .lt. 0.d0) then
                            tmp=-rtfm*temp(ndt,j)/rate_t(ndt,j)*reduction_factor_dt_heat
                        else
                            tmp= rtfp*temp(ndt,j)/rate_t(ndt,j)*reduction_factor_dt_heat
                        endif
                    else
                        tmp=dt
                    endif
                    if (verbose_update_dt)    write(iout,*) '>>>>>> update dt heat eq: dt=',dt," ;tmp=",tmp
                    if ((dt .gt. tmp) .and. (tmp .ne. 0.d0)) then
                        dt=tmp
                    endif
                enddo
...
! if(compute_spc) then
!         do k=1,nspc
!             if (Gamma_in_pulse_int(k).ne.Gamma_in_pulse_N) then
!                 if (time.ge.Gamma_in_pulse_starttime(k)) then
...
! 
! 
!             if (Q_in_pulse_int.ne.Q_in_pulse_N) then
!                 if (time.ge.Q_in_pulse_starttime) then
...
!             endif
! 
!              if (T_pulse_int.ne.T_pulse_N) then
!                 if (time.ge.T_pulse_starttime) then
